...
struct EditorialView: View {
    @State private var editorialItems: [EditorialItem] = []
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    if isLoading {
                        ProgressView("Fetching news room...")
                            ...
                            .padding(.top, 40)
                    } else if editorialItems.isEmpty {
                        VStack(spacing: 12) {
                        ...
                        ForEach(Array(editorialItems.enumerated()), id: \.element.id) { index, article in
                            VStack(spacing: 16) {
                                Button(action: {
                                ...
                                if (index + 1) % 2 == 0 && index != editorialItems.count - 1 {
                                    InlineBannerAdView(
            ...
            .modifier(NavigationWrapperModifier(selectedArticle: $selectedArticle, allArticles: editorialItems))
        }
    }
    ...
    private func loadEditorial() async {
        ...
        do {
            editorialItems = try await ExcelCMSService.shared.fetchEditorial()
        } catch {
            editorialItems = []
        }
    }
